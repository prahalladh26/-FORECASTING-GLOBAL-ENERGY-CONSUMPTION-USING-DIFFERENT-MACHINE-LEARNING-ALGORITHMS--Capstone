...
dde1= Data %>% select(country,year,population,gdp,oil_electricity,oil_production,oil_consumption) %>% filter(country == "China" & year >= 1990 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
...
ggplot(dde1, aes(x = year, y = oil_consumption)) +
  ...
  scale_y_continuous(breaks = seq(1000, 10000, by = 400)) +
  ggtitle("Oil Consumption - China") +
  xlab("Year") +
