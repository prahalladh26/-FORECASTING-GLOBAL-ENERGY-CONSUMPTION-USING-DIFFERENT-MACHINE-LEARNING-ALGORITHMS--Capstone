---
title: "Global Solar Consumption"
output: 
...
dde1= Data%>%select(country,year,population,gdp,solar_electricity,
                    ...
                    solar_consumption)
dde1 <- dde1[complete.cases(dde1),]%>% filter(year >= 1990 & year <=2018) %>% filter(solar_consumption > 0)
...
library(neuralnet)
dde_model <- neuralnet(solar_consumption~., data = dde_train[,-1])
dde_model

dde_test_0 <- dde_test %>% filter(solar_consumption >0.03)
View(dde_test_0)
summary(dde_test_0)
...
cor(dde_test_0$solar_consumption, predicted_cons)
...
(Line<- ggplot(dde_test_0
                        , aes(Year,solar_consumption)) +
            geom_point() +
            scale_x_continuous(breaks = seq(1994, 2018, by = 2)) +
            xlab("Year") +
            ylab("Solar Consumption") +
