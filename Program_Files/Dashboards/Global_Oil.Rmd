...
#loading the library 
library(dplyr)
dde1= Data%>%select(country,year,population,gdp,oil_electricity,oil_production,oil_consumption)
dde1 <- dde1[complete.cases(dde1),]%>% filter(year >= 1990 & year <=2018) %>% filter(oil_consumption > 0)
...
dde_model <- neuralnet(oil_consumption~., data = dde_train[,-1])
dde_model

dde_test_0 <- dde_test %>% filter(oil_consumption >0.03)
View(dde_test_0)
summary(dde_test_0)

# model results

model_results <- compute(dde_model,dde_test_0[,-6])
model_results 

(predicted_cons <- model_results$net.result)
cor(dde_test_0$oil_consumption, predicted_cons)


VALIDATION=table(dde_test_0[,6],predicted_cons)
...
### Predicted Yearly Oil Consumption
