---
title: "Global Hydro-Power Consumption"
output: 
...
#loading the library 
library(dplyr)
dde1= Data%>%select(country,year,population,gdp,hydro_electricity,
                    hydro_cons_change_twh,hydro_share_elec,hydro_share_energy,
                    hydro_consumption)
#dde1 <- dde1[complete.cases(dde1),]%>% filter(country=="India",year >= 1990 & year <=2018) %>% filter(hydro_consumption > 0)
dde1 <- dde1[complete.cases(dde1),]%>% filter(year >= 1990 & year <=2018) %>% filter(hydro_consumption > 0)
...
dde_model <- neuralnet(hydro_consumption~., data = dde_train[,-1])
dde_model




dde_test_0 <- dde_test %>% filter(hydro_consumption >0.03)
View(dde_test_0)
summary(dde_test_0)
...
cor(dde_test_0$hydro_consumption, predicted_cons)
...
(ggplotly(Line<- ggplot(dde_test_0
                        , aes(Year,hydro_consumption)) +
            geom_point() +
            scale_x_continuous(breaks = seq(1994, 2018, by = 4)) +
            xlab("Year") +
            ylab("hydro-energy Consumption") +
