---
title: "Global Nuclear Consumption"
output: 
...
#loading the library 
library(dplyr)
dde1= Data%>%select(country,year,population,gdp,nuclear_electricity,nuclear_cons_change_twh,nuclear_share_elec,nuclear_share_energy,nuclear_consumption)
dde1 <- dde1[complete.cases(dde1),]%>% filter(year >= 1990 & year <=2018) %>% filter(nuclear_consumption > 0)
...
dde_test_0 <- dde_test %>% filter(nuclear_consumption >0.03)
View(dde_test_0)
summary(dde_test_0)
...
cor(dde_test_0$nuclear_consumption, predicted_cons)
...
(Line<- ggplot(dde_test_0
                        , aes(Year,nuclear_consumption)) +
            geom_point() +
            scale_x_continuous(breaks = seq(1994, 2018, by = 4)) +
            xlab("Year") +
            ylab("Nuclear Consumption") +
