...
#loading the library 
library(dplyr)
dde1= Data%>%select(country,year,population,gdp,gas_electricity,
                    gas_production,gas_share_elec,gas_share_energy,
                    gas_consumption)
dde1 <- dde1[complete.cases(dde1),]%>% filter(year >= 1990 & year <=2018) %>% filter(gas_consumption > 0)
...
dde_model
dde_test_0 <- dde_test %>% filter(gas_consumption >0.03)
View(dde_test_0)
summary(dde_test_0)
...
cor(dde_test_0$gas_consumption, predicted_cons)
