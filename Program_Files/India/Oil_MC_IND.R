...
dde1= Data %>% select(country,year,population,gdp,oil_electricity,oil_production,oil_consumption) %>% filter(country == "India" & year >= 1985 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
...
ggplot(dde1, aes(x = year, y = oil_consumption)) +
  ...
  scale_y_continuous(breaks = seq(500, 3000, by = 100)) +
  ggtitle("Oil Consumption - India") +
  xlab("Year") +
