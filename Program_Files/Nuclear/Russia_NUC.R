...
library(dplyr)
dde1= Data %>% select(country,year,population,gdp,nuclear_electricity,nuclear_cons_change_twh,nuclear_share_elec,nuclear_share_energy,nuclear_consumption) %>% filter(country == "Russia" & year >= 1986 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
...
dde_model <- neuralnet(nuclear_consumption~., data = dde_train[,-1], hidden = c(3,2))
dde_model
...
ggplot(dde1, aes(x = year, y = nuclear_consumption)) +
  ...
  ggtitle("Nuclear Consumption - Russia") +
  xlab("Year") +
