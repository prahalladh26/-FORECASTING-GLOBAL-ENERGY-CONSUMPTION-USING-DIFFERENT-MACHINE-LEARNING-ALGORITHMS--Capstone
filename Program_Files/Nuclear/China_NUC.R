...
library(dplyr)
dde1= Data %>% select(country,year,population,gdp,nuclear_electricity,nuclear_cons_change_twh,nuclear_share_elec,nuclear_share_energy,nuclear_consumption) %>% filter(country == "United States" & year >= 1993 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
...
dde_model <- neuralnet(nuclear_consumption~., data = dde_train, hidden = c(3,2))
dde_model
...
ggplot(dde1, aes(x = year, y = nuclear_consumption)) +
  ...
  scale_y_continuous(breaks = seq(10, 1100, by = 100)) +
  ggtitle("Nuclear Consumption - China") +
  xlab("Year") +
...
ggplot(data_mod,                                  
  ...
  geom_abline(intercept = 0,
              ...
              color = "red",
              size = 1)+
  ggtitle("Nuclear Energy Consumption - China") +
  xlab("Predicted") +
  ylab("Actual") 
