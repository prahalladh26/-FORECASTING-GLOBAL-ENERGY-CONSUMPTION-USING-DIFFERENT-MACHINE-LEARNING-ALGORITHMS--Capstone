...
library(dplyr)
dde1= Data %>% select(country,year,population,gdp,solar_electricity,
                      solar_cons_change_twh,solar_share_elec,solar_share_energy,
                      solar_consumption) %>% filter(country == "Netherlands" & year >= 1995 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
...
dde_model <- neuralnet(solar_consumption~., data = dde_train[,-1], hidden = c(3,2))
dde_model
...
ggplot(dde1, aes(x = year, y = solar_consumption)) +
  ...
  scale_y_continuous(breaks = seq(220, 600, by = 30)) +
  ggtitle("Solar Consumption - Netherlands") +
  xlab("Year") +
  ylab("Primary Energy Consumption sourced from Solar")
