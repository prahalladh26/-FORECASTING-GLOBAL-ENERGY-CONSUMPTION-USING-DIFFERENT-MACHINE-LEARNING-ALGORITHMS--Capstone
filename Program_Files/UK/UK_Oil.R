...
dde1= Data %>% select(country,year,population,gdp,oil_electricity,oil_production,oil_consumption) %>% filter(country == "United Kingdom" & year >= 1990 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
dde <- dde1[,-1] #removed both Country and year column
...
ggplot(dde1, aes(x = year, y = oil_consumption)) +
  ...
  scale_y_continuous(breaks = seq(800, 1000, by = 25)) +
  ggtitle("Oil Consumption - United kingdom") +
  xlab("Year") +
