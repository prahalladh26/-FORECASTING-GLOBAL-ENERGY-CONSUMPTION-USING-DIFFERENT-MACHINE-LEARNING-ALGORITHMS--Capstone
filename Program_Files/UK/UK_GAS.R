...
library(dplyr)
dde1= Data %>% select(country,year,population,gdp,gas_electricity,gas_production,gas_share_elec,gas_share_energy,gas_consumption) %>% filter(country == "United Kingdom" & year >= 1990 & year <=2018)
View(dde1)
summary(dde1)
...
ggplot(dde1, aes(x = year, y = gas_consumption)) +
  ...
  scale_y_continuous(breaks = seq(500, 1100, by = 100)) +
  ggtitle("Gas Consumption - United Kingdom") +
  xlab("Year") +
