...
library(dplyr)
dde1= Data %>% select(country,year,population,gdp,coal_electricity,coal_production,coal_consumption) %>% filter(country == "United Kingdom" & year >= 1986 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
...
ggplot(dde1, aes(x = year, y = coal_consumption)) +
  ...
  scale_y_continuous(breaks = seq(50, 800, by = 50)) +
  ggtitle("Coal Consumption - United Kingdom") +
  xlab("Year") +
