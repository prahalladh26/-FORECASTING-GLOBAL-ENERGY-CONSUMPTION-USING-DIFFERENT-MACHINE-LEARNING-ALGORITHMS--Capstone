...
library(dplyr)
dde1= Data %>% select(country,year,population,gdp,wind_electricity,wind_cons_change_twh,wind_share_elec,wind_share_energy,wind_consumption) %>% filter(country == "India" & year >= 1985 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
...
dde_model <- neuralnet(wind_consumption~., data = dde_train , hidden = c(2))
dde_model
...
library(ggplot2)
ggplot(dde1, aes(x = year, y = wind_consumption)) +
  geom_line(color = "red") +
...
cor(dde_test$wind_consumption, predicted_cons)
