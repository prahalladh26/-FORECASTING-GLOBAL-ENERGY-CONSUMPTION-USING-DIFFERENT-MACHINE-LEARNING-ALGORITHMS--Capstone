...
library(dplyr)
dde1= Data %>% select(country,year,population,gdp,gas_electricity,gas_production,gas_share_elec,gas_share_energy,gas_consumption) %>% filter(country == "United States" & year >= 1990 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
...
ggplot(dde1, aes(x = year, y = gas_consumption)) +
 ...
 scale_y_continuous(breaks = seq(8500, 12000, by = 400)) +
  ggtitle("Gas Consumption - United States") +
  xlab("Year") +
