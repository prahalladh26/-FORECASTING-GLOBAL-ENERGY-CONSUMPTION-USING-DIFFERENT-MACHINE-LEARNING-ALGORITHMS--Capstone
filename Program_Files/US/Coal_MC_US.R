...
library(dplyr)
dde1= Data %>% select(country,year,population,gdp,coal_electricity,coal_production,coal_consumption) %>% filter(country == "United States" & year >= 1985 & year <=2018)
View(dde1)
summary(dde1)
str(dde1)
...
ggplot(dde1, aes(x = year, y = coal_consumption)) +
  ...
  scale_y_continuous(breaks = seq(3000, 7000, by = 300)) +
  ggtitle("Actual Coal Consumption - United States") +
  xlab("Year") +
